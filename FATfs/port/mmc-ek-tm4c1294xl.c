...
/* Card detect pin */
#define SDC_CD_GPIO_PORT_BASE   	GPIO_PORTK_BASE
#define SDC_CD             			GPIO_PIN_0
/* Write protect pin */
#define SDC_WP_GPIO_PORT_BASE   	GPIO_PORTK_BASE
#define SDC_WP             			GPIO_PIN_1
...
static volatile ULONG	  Timer            = 0;           	/* Read/Write timer */
...
/*-----------------------------------------------------------------------*/
static
void send_initial_clock_train(int quantity)
{
    unsigned int i;
    uint32_t ui32Dat;
...
/*-----------------------------------------------------------------------*/
/* Card Detect and write protection pin config  (Platform dependent)     */
/*-----------------------------------------------------------------------*/
void init_control_pins(void)
{
    ROM_SysCtlPeripheralEnable(SYSCTL_PERIPH_GPIOK);
    GPIOPinTypeGPIOInput(SDC_CD_GPIO_PORT_BASE, SDC_CD | SDC_WP);
    GPIOPadConfigSet(SDC_CD_GPIO_PORT_BASE, SDC_CD, GPIO_STRENGTH_8MA_SC, GPIO_PIN_TYPE_STD_WPU);
    GPIOPadConfigSet(SDC_WP_GPIO_PORT_BASE, SDC_WP, GPIO_STRENGTH_8MA_SC, GPIO_PIN_TYPE_STD_WPU);
}
...
void power_on (void)
{
    ...
    SSIConfigSetExpClk(SDC_SSI_BASE, configCPU_CLOCK_HZ,
...
void set_max_speed(void)
{
    ...
    ROM_SSIConfigSetExpClk(SDC_SSI_BASE, configCPU_CLOCK_HZ,
...
)
{
    ...
    do {                            /* Receive the data block into buffer */
        ...
        *buff++ = HWREG(SDC_SSI_BASE + SSI_O_DR);
...
)
{
    ...
    send_initial_clock_train(10);            /* Ensure the card is in SPI mode */
    SELECT();                /* CS = L */
    ty = 0;
...
void set_disk_status (BYTE drv, DSTATUS status)
{
...
void disk_timerproc (void)
{
	...
	if ((GPIOPinRead(SDC_WP_GPIO_PORT_BASE, SDC_WP)&SDC_WP) == SDC_WP)
	{
	...
	else
	{
	  ...
	  if ((GPIOPinRead(SDC_CD_GPIO_PORT_BASE, SDC_CD)&SDC_CD) == SDC_CD)
		s |= (STA_NODISK | STA_NOINIT);		/* Socket empty */
	else
		s &= ~STA_NODISK;	  				/* Card detected */
...
void GetFatTimer(unsigned long *time)
{
  taskENTER_CRITICAL();
  *time = Timer;
  taskEXIT_CRITICAL();
...
void SetFatTimer(unsigned long time)
{
  taskENTER_CRITICAL();
  Timer = time;
  taskEXIT_CRITICAL();
